...
Testing some functions to extrapolate TAS precip data using nearby MLR precip data.
...
Import NDVI time series calculated in accompanying Google Earth Engine python notebook:
...
```
Now highlight the late spring observations (corresponding to peak annual grasses) and the late fall (when annual grasses are most senescent so woody vegetation is highlighted):
```{r message=FALSE, warning=FALSE}
...
```
Fit a trend (change in NDVI over time) for each month, before and after restoration: 
```{r message=FALSE, warning=FALSE}
ndvi_time_series_interp %>% 
  mutate(time_month = fct_recode(as.factor(month(date_round)), 
                                 "Jan" = "1", "Feb" = "2" , "Mar" = "3", 
...
Testing this but no clear signal. Trying to find if NDVI changes vary significantly by floodplain elevation:
